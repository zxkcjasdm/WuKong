...
if __name__=="__main__":
    ...
    sigs = contract_instance.functions.callSign(x_array,labs_array,sk,pk).call({'gas': GAS_PROVIDED})
    #tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
    print("Sigs:",sigs)
    ...
    class DataProvider:
        def __init__(self,pk,sk,t) -> None:
            ...
            self.sigs=contract_instance.functions.callSign(self.data,self.labs,self.sk,self.pk).call({'gas': GAS_PROVIDED})
    n=5
    func=0 #Avg函数
    DPs=[]
    ...
    count=0
    for dp in DPs:
        vk_r,proof,computed_value=contract_instance.functions.callEval(dp.data,dp.labs,dp.pk,dp.sigs,func).call({'gas': GAS_PROVIDED})
    ...
    sigs_psc = contract_instance.functions.callSign(computed_values,labs_psc,sk_psc,pk_psc).call({'gas': GAS_PROVIDED})
    vk_r_psc,proof_psc,result=contract_instance.functions.callEval(computed_values,labs_psc,pk_psc,sigs_psc,func).call({'gas': GAS_PROVIDED})
